//@formatter:off
package arrow.businesstraceability.persistence.dto;

/*=================== Start import section after the marker line ==================*/
/*=================== Please ensure all new imports go into this section ==================*/

import arrow.businesstraceability.persistence.mapped.Branch_position_MAPPED;

import arrow.businesstraceability.persistence.entity.Branch_position;

/*=================== End of import section before the marker line ===================*/
/*=================== There must be one blank line before the marker line ===================*/

public class Branch_position_DTO extends Branch_position {
    private Branch_position_MAPPED.Pk pk;

    public void setPk(final Branch_position_MAPPED.Pk pk) {
        this.pk = pk;
    }
    
    @Override
    public Branch_position_MAPPED.Pk getPk() {
        return this.pk;
    }

    private int pos_id;

    @Override
    public int getPos_id() {
        return this.pos_id;
    }

    public void setPos_id(final int pos_id) {
        this.pos_id = pos_id;
    }
  

    private boolean isPersisted;

    @Override
    public boolean isPersisted() {
        return this.isPersisted;
    }

    public void setPersisted(final boolean value) {
        this.isPersisted = value;
    }

    //@formatter:on
    /* =================== Start of manually added code after the marker line ================== */



    /* =================== End of manually added code before the marker line =================== */
    //@formatter:off
}
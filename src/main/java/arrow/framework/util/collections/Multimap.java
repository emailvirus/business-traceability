...
/**
 * A collection similar to a {@code Map}, but which may associate multiple values with a single key. If you call
 * {@link #put} twice, with the same key but different values, the multimap contains mappings from the key to both
 * values.
 * <p/>
 * <p/>
 * The methods {@link #get}, {@link #keySet}, {@link #keys}, {@link #values}, {@link #entries}, and {@link #asMap}
 * return collections that are views of the multimap. If the multimap is modifiable, updating it can change the contents
 * of those collections, and updating the collections will change the multimap. In contrast, {@link #replaceValues} and
 * {@link #removeAll} return collections that are independent of subsequent multimap changes.
 * <p/>
 * <p/>
 * Depending on the implementation, a multimap may or may not allow duplicate key-value pairs. In other words, the
 * multimap contents after adding the same key and value twice varies between implementations. In multimaps allowing
 * duplicates, the multimap will contain two mappings, and {@code get} will return a collection that includes the value
 * twice. In multimaps not supporting duplicates, the multimap will contain a single mapping from the key to the value,
 * and {@code get} will return a collection that includes the value once.
 * <p/>
 * <p/>
 * All methods that alter the multimap are optional, and the views returned by the multimap may or may not be
 * modifiable. When modification isn't supported, those methods will throw an {@link UnsupportedOperationException}.
 *
 * @author Jared Levy
...
public interface Multimap<K, V> {
     ...
     * returned map, such as element removal, will update the underlying multimap. The map does not support
     ...
     * <p/>
     * <p/>
     * In general, two multimaps with identical key-value mappings may or may not be equal, depending on the
     * implementation. For example, two {@link SetMultimap} instances with the same key-value mappings are equal, but
     * equality of two ListMultimap instances depends on the ordering of the values for each key.
     * <p/>
     * <p/>
     * A non-empty {@link SetMultimap} cannot be equal to a non-empty ListMultimap, since their {@link #asMap} views
     * contain unequal collections as values. However, any two empty multimaps are equal, because they both have empty
     * {@link #asMap} views.
     *
     * @param obj the obj
     * @return true, if successful
